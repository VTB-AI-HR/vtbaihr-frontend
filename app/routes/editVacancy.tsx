...
const EditVacancy: React.FC = () => {
  ...
  const vacancyId = vacancy_id ? Number(vacancy_id) : undefined;
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!vacancyId) return;
      try {
        const [vacanciesRes, resumeWeightsRes, interviewWeightsRes] = await Promise.all([
          axios.get("https://vtb-aihr.ru/api/vacancy/all"),
          axios.get(`https://vtb-aihr.ru/api/vacancy/resume-weights/${vacancyId}`),
          axios.get(`https://vtb-aihr.ru/api/vacancy/interview-weights/${vacancyId}`),
        ]);

        const vacancies = vacanciesRes.data?.vacancies || vacanciesRes.data || [];
        const current = Array.isArray(vacancies)
          ? vacancies.find((v: any) => Number(v.id ?? v.vacancy_id) === vacancyId)
          : undefined;
        ...
        if (current) {
          setForm({
            ...
            skill_lvl: current.skill_lvl ?? undefined,
          });
        }

        const resume = resumeWeightsRes.data.find((r: any) => r.vacancy_id === vacancyId) ?? {};
        setReviewWeights({
          accordance_xp_vacancy_score_threshold: Number(resume.accordance_xp_vacancy_score_threshold ?? 3),
          accordance_skill_vacancy_score_threshold: Number(resume.accordance_skill_vacancy_score_threshold ?? 3),
          recommendation_weight: Number(resume.recommendation_weight ?? 3),
          portfolio_weight: Number(resume.portfolio_weight ?? 3),
        });

        const interview = interviewWeightsRes.data.find((i: any) => i.vacancy_id === vacancyId) ?? {};
        console.log(interview)
        setInterviewWeights({
          logic_structure_score_weight: Number(interview.logic_structure_score_weight ?? 3),
          soft_skill_score_weight: Number(interview.soft_skill_score_weight ?? 3),
          hard_skill_score_weight: Number(interview.hard_skill_score_weight ?? 3),
          accordance_xp_resume_score_weight: Number(interview.accordance_xp_resume_score_weight ?? 3),
          accordance_skill_resume_score_weight: Number(interview.accordance_skill_resume_score_weight ?? 3),
          red_flag_score_weight: Number(interview.red_flag_score_weight ?? 3),
        });
      } catch (e) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      await axios.put("https://vtb-aihr.ru/api/vacancy/edit", {
        vacancy_id: vacancyId,
        ...form,
      });

      await axios.put("https://vtb-aihr.ru/api/vacancy/resume-weights/edit", {
        vacancy_id: vacancyId,
        ...reviewWeights,
      });

      await axios.put("https://vtb-aihr.ru/api/vacancy/interview-weights/edit", {
        vacancy_id: vacancyId,
        ...interviewWeights,
      });
